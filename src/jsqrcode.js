// REPACKAGED VERSIONS OF JSQRCODE WITH FIXES
// ORIGINAL https://github.com/LazarSoft/jsqrcode
// FIXED https://github.com/Thulinma/jsqrcode/tree/finder_fix_mini (By Thulinma)
...
let FORMAT_INFO_DECODE_LOOKUP = [[0x5412, 0x00], [0x5125, 0x01], [0x5E7C, 0x02], [0x5B4B, 0x03], [0x45F9, 0x04], [0x40CE, 0x05], [0x4F97, 0x06], [0x4AA0, 0x07], new Array(0x77C4, 0x08), new Array(0x72F3, 0x09), new Array(0x7DAA, 0x0A), new Array(0x789D, 0x0B), new Array(0x662F, 0x0C), new Array(0x6318, 0x0D), new Array(0x6C41, 0x0E), new Array(0x6976, 0x0F), new Array(0x1689, 0x10), new Array(0x13BE, 0x11), new Array(0x1CE7, 0x12), new Array(0x19D0, 0x13), new Array(0x0762, 0x14), new Array(0x0255, 0x15), new Array(0x0D0C, 0x16), new Array(0x083B, 0x17), new Array(0x355F, 0x18), new Array(0x3068, 0x19), new Array(0x3F31, 0x1A), new Array(0x3A06, 0x1B), new Array(0x24B4, 0x1C), new Array(0x2183, 0x1D), new Array(0x2EDA, 0x1E), new Array(0x2BED, 0x1F)];
let BITS_SET_IN_HALF_BYTE = [0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4];
...
qrcode.grayScaleToBitmap=function(grayScale)
{
    ...
    let bitmap = new Array(qrcode.height*qrcode.width);
	...
	this.foundPatternCross=function( stateCount)
		{
			...
			let maxletiance = Math.floor(moduleSize / 2);
			// Allow less than 50% letiance from 1-1-3-1-1 proportions
			return Math.abs(moduleSize - (stateCount[0] << INTEGER_MATH_SHIFT)) < maxletiance && Math.abs(moduleSize - (stateCount[1] << INTEGER_MATH_SHIFT)) < maxletiance && Math.abs(3 * moduleSize - (stateCount[2] << INTEGER_MATH_SHIFT)) < 3 * maxletiance && Math.abs(moduleSize - (stateCount[3] << INTEGER_MATH_SHIFT)) < maxletiance && Math.abs(moduleSize - (stateCount[4] << INTEGER_MATH_SHIFT)) < maxletiance;
		}
	...
	this.haveMultiplyConfirmedCenters=function()
		{
			...
			// lety too much. We arbitrarily say that when the total deviation from average exceeds
	...
	this.foundPatternCross = function(stateCount)
		{
			...
			let maxletiance = moduleSize / 2.0;
			for (let i = 0; i < 3; i++)
			{
				if (Math.abs(moduleSize - stateCount[i]) >= maxletiance)
				{
					return false;
				}
			}
